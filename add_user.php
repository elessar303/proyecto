...
$consulta=$estatus->consulta("select a.id, a.login, a.clave, a.nombres, a.apellidos, a.cedula, a.cod_dep, b.descripcion,b.id_departamento from usuarios as a, departamentos as b where a.cod_dep=b.id_departamento and a.id='".$_GET['usuario']."'");
    ...
    $fila=$estatus->fetch_array($consulta);
           ...
           echo"<tr class='alt'>";
            ...
            echo"
        <td>
          <input type=text style='width: 100%; font-size: 12;'   id='login' name ='login' value='".utf8_encode($fila['login'])."' />
        </td>
        <td>
          <input type=text style='width: 100%; font-size: 12;' name=nombre id=nombre value='".$fila['nombres']."'/>
        </td>
        <td>
          <input type=text style='width: 100%; font-size: 12;' name='apellidos' id='apellidos' value='".$fila['apellidos']."'/>
        </td>
        <td>
          <input type=text style='width: 100%; font-size: 12;' name='cedula' id='cedula' value='".$fila['cedula']."'/>
        </td>
        <td>
          <input type=password style='width: 100%; font-size: 12;' name='clave' id='clave' value='".$fila['clave']."'/>
        </td>
        <td>
          <select name=departamento id=departamento size=1 ;'>";$consulta_dep=$conn->consulta("select id_departamento, descripcion from departamentos");
        ...
        <td><button id=actuar   nombre=actuar value=".$fila['id']." onclick='modificar()'>MODIFICAR</button>
